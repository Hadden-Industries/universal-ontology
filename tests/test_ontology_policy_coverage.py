...
class CoverageReconciliationTest(unittest.TestCase):
    def setUp(self):
        ...
        self.ledger: CoverageLedger = reconcile_coverage(
            self.policy, render_editing_policy(self.policy)
        )

    def test_every_executable_rule_is_documented_with_positive_and_negative_fixtures(
        self,
    ):
    ...
    def test_every_human_clause_is_documented_and_has_no_fixture_claim(self):
        ...
        self.assertFalse(
            self.ledger.human & (self.ledger.positive | self.ledger.negative)
        )

    def test_a_fixture_claiming_an_unknown_rule_is_reported(self):
        ledger = reconcile_coverage(
            self.policy,
            render_editing_policy(self.policy),
            extra_claims={"EP-DOES-NOT-EXIST": {"positive"}},
        )
        self.assertTrue(
            any("EP-DOES-NOT-EXIST" in problem for problem in ledger.problems)
        )

    def test_a_duplicate_and_an_omission_do_not_cancel_out(self):
        omitted = sorted(self.ledger.executable)[-1]
        same_size_wrong_members = (set(self.ledger.executable) - {omitted}) | {
            "EP-EXTRA-DUPLICATE"
        }
        ...
        ledger = reconcile_coverage(
            self.policy,
            render_editing_policy(self.policy),
            override_negative=same_size_wrong_members,
        )
