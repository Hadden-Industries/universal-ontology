...
from ontology_policy.changes import (  # noqa: E402
    ChangeKind,
    classify_changes,
    rooted_closure,
)
from ontology_policy.snapshots import (  # noqa: E402
    parse_module_bytes,
    read_module_source,
)
...
def graph(turtle: str, name="g.ttl"):
    return parse_module_bytes(
        FIXTURE_MODULE, name, (PREFIXES + turtle).encode("utf-8"), "turtle"
    )


def kinds(previous: str, current: str):
    facts = classify_changes(
        graph(current, "current.ttl"), graph(previous, "previous.ttl")
    )
...
"""
    ...
    def test_outgoing_assertion_change_marks_only_the_subject(self):
        changed = BASE.replace(
            'ex:B a owl:Class ; rdfs:label "B"@en .',
            'ex:B a owl:Class ; rdfs:label "B renamed"@en .',
        )
    ...
    def test_axiom_annotation_edit_changes_the_annotated_source(self):
        changed = BASE.replace(
            'rdfs:comment "note"@en', 'rdfs:comment "different note"@en'
        )
        result = kinds(BASE, changed)
...
"""
    ...
    def test_added_deleted_and_revert(self):
        ...
        self.assertEqual(
            kinds(BASE, BASE.replace('"B"@en', '"B"@en')), kinds(BASE, BASE)
        )
        self.assertTrue(
            all(kind == ChangeKind.UNCHANGED for kind in kinds(BASE, BASE).values())
        )
    ...
    def test_closure_contents_are_exactly_the_owned_structure(self):
        ...
        predicates = {
            str(p).rsplit("#", 1)[-1].rsplit("/", 1)[-1] for _, p, _ in closure
        }
        self.assertTrue(
            {
                "type",
...
class GitSnapshotSelectionTest(unittest.TestCase):
    ...
    def test_staged_and_worktree_bytes_are_distinguished_in_both_directions(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            subprocess.run(
                ["git", "-C", str(repo), "config", "user.email", "t@example.org"],
                check=True,
            )
            subprocess.run(
            ...
            committed = read_module_source(
                FIXTURE_MODULE, "module.ttl", revision="HEAD", repository=repo
            )
            # stage a change, then alter the worktree differently
            path.write_text(
                PREFIXES + BASE.replace('"B"@en', '"B staged"@en'), encoding="utf-8"
            )
            subprocess.run(["git", "-C", str(repo), "add", "module.ttl"], check=True)
            path.write_text(
                PREFIXES + BASE.replace('"B"@en', '"B worktree"@en'), encoding="utf-8"
            )
            staged = read_module_source(
                FIXTURE_MODULE, "module.ttl", revision="", repository=repo
            )
            ...
            self.assertEqual(
                classify_changes(staged, committed)[URIRef(EX + "B")],
                ChangeKind.CHANGED,
            )
            self.assertEqual(
                classify_changes(committed, committed)[URIRef(EX + "B")],
                ChangeKind.UNCHANGED,
            )
            # a renamed path with identical bytes is not a graph change
            renamed = read_module_source(
                FIXTURE_MODULE, "module.ttl", revision="HEAD", repository=repo
            )
            self.assertTrue(
                all(
                    kind == ChangeKind.UNCHANGED
                    for kind in classify_changes(renamed, committed).values()
                )
            )
            ...
            with self.assertRaises(InputError):
                read_module_source(
                    FIXTURE_MODULE, "missing.ttl", revision="HEAD", repository=repo
                )
...
"""
    ...
    def run_policy(self, purpose, comparisons, policy=None, **kwargs):
        ...
        return validate_sources(
            ...
            authorities_directory=self.authorities,
    ...
    def modified_results(self, outcome):
        return {
            r.focus_node.replace(EX, "ex:")
            for r in outcome.results
            if r.requirement_id == "EP-MODIFIED"
        }

    def test_changed_entity_without_modified_fails_while_unchanged_value_new_entity_and_untouched_pass(
        self,
    ):
    ...
    def test_without_a_comparison_the_obligation_is_reported_unevaluated_not_invented(
        self,
    ):
        from ontology_policy import RunPurpose
        ...
        self.assertEqual(
            outcome.unevaluated_change_obligations, (str(FIXTURE_MODULE.iri),)
        )
    ...
    def test_candidate_qualification_requires_a_comparison(self):
        ...
        outcome = self.run_policy(
            RunPurpose.CANDIDATE, {FIXTURE_MODULE.iri: self.previous}
        )
        self.assertIn("ex:B", self.modified_results(outcome))
        self.assertEqual(outcome.status, 1)

    def test_removing_the_shacl_applicability_removes_the_obligation_although_the_facts_remain(
        self,
    ):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            outcome = self.run_policy(
                RunPurpose.DRAFT,
                {FIXTURE_MODULE.iri: self.previous},
        ...
        _, context = build_validation_graph(
            [self.current], RunPurpose.DRAFT, {FIXTURE_MODULE.iri: self.previous}
        )
        ...
        self.assertIn(
            (URIRef(EX + "F"), UOC.refersToDeleted, URIRef(EX + "D")), context
        )
    ...
    def test_critical_fix_scope_limits_results_and_never_qualifies(self):
        ...
        with self.assertRaises(ContextError):
            self.run_policy(
                RunPurpose.CRITICAL_FIX, {FIXTURE_MODULE.iri: self.previous}
            )
        outcome = self.run_policy(
            RunPurpose.CRITICAL_FIX,
            {FIXTURE_MODULE.iri: self.previous},
            scope_reference="Issue #99 approved scope",
        )
        focus = {r.focus_node.replace(EX, "ex:") for r in outcome.results}
        self.assertIn("ex:B", focus)
        self.assertNotIn(
            "ex:G", focus, "an untouched historic defect is outside the fix scope"
        )
        self.assertFalse(outcome.qualifies)
...
class ImportPinCoherenceTest(unittest.TestCase):
    ...
    def make(self, label, ontology, version, imports=()):
        ...
        module = OwnedModule(
            iri=URIRef(f"https://example.org/policy-fixture/module/{label}"),
            label=label,
            ontology_iri=URIRef(ontology),
            owned_namespaces=(ontology,),
            working_path=f"{label}.ttl",
    ...
    def test_coherent_pins_pass_and_a_stale_or_newer_pin_is_a_context_error(self):
        ...
        core = self.make(
            "core", "https://example.org/core/", "https://example.org/core/20260912"
        )
        extended = self.make(
            "extended",
            "https://example.org/extended/",
            "https://example.org/extended/20260912",
            imports=[
                "https://example.org/core/20260912",
