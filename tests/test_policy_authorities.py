...
FIXTURE_RAW = (
    REPOSITORY_ROOT / "tests" / "fixtures" / "ontology-policy" / "authorities" / "raw"
)
...
def rights(source_name: str) -> dict:
    return {
        ...
        "rightsDecision": "fixture decision reference",
        "retrievedOn": "2026-09-12",
    }


class IngestionTest(unittest.TestCase):
    def test_each_authority_derives_members_with_provenance_from_its_native_format(
        self,
    ):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            for name, specification in AUTHORITY_SPECIFICATIONS.items():
                ...
                snapshot = derive_snapshot(
                    name, raw.read_bytes(), Path(temporary), rights(name)
                )
                self.assertEqual(snapshot.member_count, expected_counts[name], name)
                provenance = json.loads(
                    (Path(temporary) / f"{name}.provenance.json").read_text(
                        encoding="utf-8"
                    )
                )
                self.assertEqual(
                ...
                self.assertEqual(
                    provenance["derivedSha256"],
                    hashlib.sha256(
                        (Path(temporary) / f"{name}.members.ttl").read_bytes()
                    ).hexdigest(),
                )
                self.assertEqual(
                    reconcile_snapshot(name, raw.read_bytes(), Path(temporary)), ()
                )
    ...
    def test_members_keep_their_exact_iris(self):
        with tempfile.TemporaryDirectory() as temporary:
            derive_snapshot(
                "loc-iso639-1",
                (FIXTURE_RAW / "loc-iso639-1.rdf").read_bytes(),
                Path(temporary),
                rights("loc"),
            )
            graph = load_authority_graph(Path(temporary), required=("loc-iso639-1",))
            members = {str(o) for o in graph.objects()} | {
    ...
    def test_a_tampered_derived_file_or_changed_payload_does_not_reconcile(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertTrue(
                reconcile_snapshot(
                    "eu-file-type", raw + b"<!-- changed -->", Path(temporary)
                )
            )
    ...
    def test_missing_or_corrupt_snapshot_is_an_error_not_an_empty_authority(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            derive_snapshot(
                "iana-media-types",
                (FIXTURE_RAW / "iana-media-types.xml").read_bytes(),
                Path(temporary),
                rights("iana"),
            )
