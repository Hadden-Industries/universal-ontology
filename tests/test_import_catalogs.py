...
def direct_imports(document: Path) -> set[str]:
    ...
    return {
        element.get(RDF_RESOURCE)
        for element in root.iter(OWL_IMPORTS)
        if element.get(RDF_RESOURCE)
    }
...
def catalog_entries(catalog: Path) -> dict[str, str]:
    ...
    return {
        element.get("name"): element.get("uri")
        for element in root.iter(f"{CATALOG_NS}uri")
        if element.get("name")
    }
...
class ImportCatalogTest(unittest.TestCase):
    ...
    def owned_import_closure(self, working_path: str) -> set[str]:
        ...
        while frontier:
            ...
            self.assertTrue(
                artifact.is_file(),
                f"{working_path} imports {iri} but src/ has no such dated artifact",
            )
    ...
    def test_every_owned_import_in_the_closure_has_a_local_catalog_entry(self):
        for working_path in self.modules:
            ...
            with self.subTest(module=working_path):
                ...
                self.assertTrue(
                    catalog.is_file(),
                    f"{working_path} has owned imports but no {catalog.name}",
                )
                ...
                for iri in sorted(closure):
                    self.assertIn(
                        iri,
                        entries,
                        f"{catalog} lacks an entry for {iri}; update it with the version increment",
                    )
                    ...
                    self.assertEqual(
                        target,
                        expected,
                        f"{catalog} maps {iri} to {target}, expected {expected}",
                    )
        ...
        for working_path in self.modules:
            ...
            with self.subTest(module=working_path):
                for name, target in catalog_entries(catalog).items():
                    self.assertFalse(
                        target.replace("\\", "/").startswith("../dist/"),
                        f"{catalog}: {name} resolves to build output {target}",
                    )
                    self.assertTrue(
                        (catalog.parent / target).is_file(),
                        f"{catalog}: {name} -> {target} does not exist",
                    )
    ...
    def test_catalog_entries_for_owned_versions_are_not_stale(self):
        ...
        for working_path in self.modules:
            ...
            with self.subTest(module=working_path):
                owned_entries = {
                    name
                    for name in catalog_entries(catalog)
                    if name.startswith(OWNED_PREFIX)
                    and name[len(OWNED_PREFIX) :].rsplit("/", 1)[-1].isdigit()
                }
                self.assertEqual(
                    owned_entries,
                    closure,
                    f"{catalog} owned entries differ from the import closure",
                )
