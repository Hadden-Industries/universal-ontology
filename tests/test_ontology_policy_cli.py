...
from ontology_policy.cli import (  # noqa: E402
    SelectedSource,
...
COMMAND = [
    sys.executable,
    "-B",
    str(REPOSITORY_ROOT / "scripts" / "validate_ontologies.py"),
]
FIXTURE_AUTHORITIES = str(
    REPOSITORY_ROOT
    / "tests"
    / "fixtures"
    / "ontology-policy"
...
class SourceAssemblyTest(unittest.TestCase):
    ...
    def test_working_and_dated_paths_map_to_their_module_but_full_artifacts_do_not(
        self,
    ):
        core = module_for_path(self.modules, "core/universal-core.owl")
        self.assertEqual(core.label, "Universal Core")
        self.assertEqual(
            module_for_path(self.modules, "src/universal/core/20260912").label,
            "Universal Core",
        )
        self.assertEqual(
            module_for_path(self.modules, "src/iso-iec/11179/-3/ed-4/20260912").label,
            "ISO/IEC 11179-3 (edition 4)",
        )
        self.assertIsNone(
            module_for_path(self.modules, "src/universal/core/20260912-full")
        )
    ...
    def test_latest_active_uses_exactly_the_recorded_active_artifacts(self):
        sources = assemble_sources(
            RunPurpose.LATEST_ACTIVE,
            [SelectedSource("extended/universal-extended.owl", None)],
            self.modules,
            REPOSITORY_ROOT,
        )
        locators = sorted(source.locator for source in sources)
        self.assertEqual(
            locators, sorted(module.active_artifact_path for module in self.modules)
        )

    def test_candidate_replaces_only_the_selected_module_and_keeps_the_rest_active(
        self,
    ):
        sources = assemble_sources(
        ...
        self.assertEqual(
            by_label["Universal Extended"], "extended/universal-extended.owl"
        )
        self.assertEqual(by_label["Universal Core"], "src/universal/core/20260912")
        self.assertEqual(len(sources), len(self.modules))

    def test_comparisons_come_from_the_active_artifact_or_the_base_commit_and_never_for_latest_active(
        self,
    ):
        selected = [SelectedSource("extended/universal-extended.owl", None)]
        comparisons = assemble_comparisons(
            RunPurpose.CANDIDATE, selected, self.modules, REPOSITORY_ROOT
        )
        by_label = {module.label: comparisons[module.iri] for module in self.modules}
        self.assertEqual(
            by_label["Universal Extended"].locator, "src/universal/extended/20260912"
        )
        self.assertEqual(
            by_label["Universal Core"].locator, "src/universal/core/20260912"
        )
        self.assertTrue(
            all(
                value is None
                for value in assemble_comparisons(
                    RunPurpose.LATEST_ACTIVE, selected, self.modules, REPOSITORY_ROOT
                ).values()
            )
        )
        based = assemble_comparisons(
            RunPurpose.DRAFT,
            [SelectedSource("extended/universal-extended.owl", None, "HEAD")],
            self.modules,
            REPOSITORY_ROOT,
        )
        self.assertTrue(
            by_label["Universal Extended"].locator
            != based[
                [m for m in self.modules if m.label == "Universal Extended"][0].iri
            ].locator
        )
        self.assertTrue(
            based[
                [m for m in self.modules if m.label == "Universal Extended"][0].iri
            ].locator.startswith("HEAD:")
        )

    def test_an_active_artifact_whose_bytes_drift_from_the_activation_digest_is_refused(
        self,
    ):
        import dataclasses
        ...
        self.assertTrue(
            core.active_content_digest
            and core.active_content_digest.startswith("sha256:")
        )
        drifted = tuple(
            dataclasses.replace(m, active_content_digest="sha256:" + "0" * 64)
            if m is core
            else m
            for m in self.modules
        )
        with self.assertRaisesRegex(
            ContextError, "no longer matches the activation record"
        ):
    ...
    def test_draft_passes_merge_identical_documents_and_split_differing_ones(self):
        ...
        self.assertEqual(
            draft_passes([working, promoted, extended], self.modules, REPOSITORY_ROOT),
            [[working, extended]],
        )
        with tempfile.TemporaryDirectory() as scratch:
            ...
            other.write_bytes(
                (REPOSITORY_ROOT / "core/universal-core.owl")
                .read_bytes()
                .replace(b"2026-09-12", b"2026-09-13", 1)
            )
        ...
        with self.assertRaisesRegex(ContextError, "Two selected inputs"):
            assemble_sources(
                RunPurpose.CANDIDATE, [working, promoted], self.modules, REPOSITORY_ROOT
            )
    ...
    def test_an_input_outside_the_reviewed_modules_is_a_context_error(self):
        with self.assertRaises(ContextError):
            assemble_sources(
                ...
                [
                    SelectedSource(
                        "tests/fixtures/ontology-policy/entity-created/created.ttl",
                        None,
                    )
                ],
                self.modules,
    ...
    def test_two_inputs_for_one_module_are_a_context_error(self):
        with self.assertRaises(ContextError):
            assemble_sources(
                RunPurpose.DRAFT,
                [
                    SelectedSource("core/universal-core.owl", None),
                    SelectedSource("src/universal/core/20260912", None),
                ],
                self.modules,
                REPOSITORY_ROOT,
            )
...
class CommandContractTest(unittest.TestCase):
    def test_latest_active_purpose_runs_the_complete_active_set_through_the_normal_command(
        self,
    ):
        completed = run_command(
            "--purpose", "latest-active", "--authorities", FIXTURE_AUTHORITIES
        )
    ...
    def test_missing_authority_snapshots_are_an_execution_error(self):
        completed = run_command(
            ...
            "latest-active",
            "--authorities",
            str(REPOSITORY_ROOT / "does-not-exist"),
        )
    ...
    def test_draft_run_reports_change_obligations_and_comparisons(self):
        ...
        completed = run_command(
            "--purpose", "draft", "--authorities", FIXTURE_AUTHORITIES, "--all-current"
        )
        self.assertIn(completed.returncode, (0, 1), completed.stderr)
        self.assertIn(
            "comparison https://haddenindustries.com/ontology/policy/activation/extended: src/universal/extended/20260912 sha256:",
            completed.stdout,
        )
    ...
    def test_pre_install_planning_needs_no_rdf_dependencies(self):
        ...
        tree = ast.parse(
            (REPOSITORY_ROOT / "scripts" / "validate_ontologies.py").read_text(
                encoding="utf-8"
            )
        )
        top_level = {
