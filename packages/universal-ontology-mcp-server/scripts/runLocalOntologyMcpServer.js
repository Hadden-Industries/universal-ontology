...
export function writeLocalOntologyMcpProcessFailureEvent(
  eventName,
  safeErrorCode,
) {
...
export function createLocalUniversalOntologyMcpServer({
  ...
  const mcpHandler = createUniversalOntologyMcpHttpProtocolHandler({
    ontologyQuery,
    onError: reportError,
  });
  ...
  async function routeRequest(request, response, requestContext) {
    ...
    // This deployment exposes only POST at /mcp. Reject other methods before
    // the native Node bridge can read an otherwise unbounded non-POST body.
    if (request.method !== "POST") {
...
function rejectListenerArguments(arguments_) {
...
/** Validate and start the loopback listener from existing local artifacts. */
export async function runLocalOntologyMcpServer({
  ...
  rejectListenerArguments(arguments_);
  const configuration =
