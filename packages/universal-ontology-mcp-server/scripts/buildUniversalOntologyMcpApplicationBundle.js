...
const ALLOWED_REPOSITORY_INPUT_PREFIXES = Object.freeze([
  "packages/universal-ontology-mcp-server/src/",
  "packages/universal-ontology-query/src/",
  "packages/universal-ontology-projection-policy/src/",
]);
const ALLOWED_REPOSITORY_INPUT_PATHS = new Set([
  ...
  "packages/universal-ontology-projection-policy/data/field-property-history.v1.json",
]);
...
const MCP_SERVER_AJV_PROVIDER_INPUT_PATTERN = /^dist\/ajvProvider-[^/]+\.mjs$/u;
...
async function collectAndValidateBundledPackageInputs(metafile) {
  ...
  for (const inputPath of Object.keys(metafile.inputs)) {
    ...
    if (packageName) {
      ...
      // Resolve the root manifest from the actual bundled module. A bare
      // self-reference uses Node's package resolution even when exports hide
      // package.json or a nested subdirectory declares its own package scope.
      const absoluteInputPath = await nodeFileSystem.realpath(
        resolve(REPOSITORY_ROOT_PATH, inputPath),
      );
      const resolvedManifestPath = findPackageJSON(
        packageName,
        pathToFileURL(absoluteInputPath),
      );
      if (!resolvedManifestPath) {
        throw new Error(
          `Bundled package manifest is missing for ${packageName}.`,
        );
      }
      const packageJsonPath =
        await nodeFileSystem.realpath(resolvedManifestPath);
      const packageRelativeInputPath = relative(
        dirname(packageJsonPath),
        absoluteInputPath,
      );
      if (
        packageRelativeInputPath === ".." ||
        packageRelativeInputPath.startsWith(`..${sep}`) ||
        isAbsolute(packageRelativeInputPath)
      ) {
        throw new Error(
          `Bundled input does not belong to its resolved package: ${inputPath}`,
        );
      }
      const existingPackage = bundledPackages.get(packageJsonPath);
      const bundledPackage = existingPackage ?? {
        name: packageName,
        packageJsonPath,
        inputs: [],
      };
      bundledPackage.inputs.push({
        absolutePath: absoluteInputPath,
        relativePath: normalizeRelativePath(packageRelativeInputPath),
      });
      bundledPackages.set(packageJsonPath, bundledPackage);
      continue;
    }
  ...
  return [...bundledPackages.values()].sort(({ name: left }, { name: right }) =>
    left.localeCompare(right),
  );
}

async function readBundledComponents(bundledPackages) {
  const components = await Promise.all(
    bundledPackages.map(async ({ name, packageJsonPath }) => {
      const packageMetadata = await readJsonDocument(packageJsonPath);
  ...
  for (const component of components) {
    const preceding = componentsByName.get(component.name);
    if (
      preceding &&
      (preceding.version !== component.version ||
        preceding.license !== component.license)
    ) {
      throw new Error(
        `Bundled installations disagree on component identity for ${component.name}.`,
      );
    }
...
async function readAndValidatePrebundledServerComponents(bundledPackages) {
  const ajvProviderInputs = bundledPackages
    .flatMap(({ name, inputs }) =>
      name === "@modelcontextprotocol/server" ? inputs : [],
    )
    .filter(({ relativePath }) =>
      MCP_SERVER_AJV_PROVIDER_INPUT_PATTERN.test(relativePath),
    );

  if (ajvProviderInputs.length !== 1) {
    throw new Error(
  ...
  const ajvProviderSourceText = await nodeFileSystem.readFile(
    ajvProviderInputs[0].absolutePath,
    "utf8",
  );
...
function createMcpPackageVersionProjectionPlugin(mcpPackageVersion) {
  return {
    name: "mcp-package-version-projection",
    setup(buildContext) {
      ...
      buildContext.onLoad({ filter: /package\.json$/ }, (arguments_) => {
        if (resolve(arguments_.path) !== resolve(PUBLIC_PACKAGE_JSON_PATH)) {
          return null;
        }
        ...
        return {
          contents: `${JSON.stringify({ version: mcpPackageVersion })}\n`,
          loader: "json",
        };
      });
    },
...
async function assertNoticesCoverBundledComponents(
  noticesText,
  bundledComponents,
  bundledPackages,
) {
  ...
  for (const { name, packageJsonPath } of bundledPackages) {
    ...
    if (
      !noticesText
        .replaceAll("\r\n", "\n")
        .includes(licenseText.replaceAll("\r\n", "\n").trimEnd())
    ) {
      throw new Error(
        `THIRD_PARTY_NOTICES.md omits the complete published LICENSE for ${name}.`,
      );
    }
  }
}
...
function assertBundleExcludesForbiddenContent(bundleBytes) {
  ...
  const forbiddenTextFragments = [
    ...
    "packages/universal-ontology-mcp-server/tests/",
    "packages/universal-ontology-query/tests/",
    "packages/universal-ontology-projection-policy/tests/",
    "sourceMappingURL=",
    "sourcesContent",
    "-----BEGIN PRIVATE KEY-----",
  ];
...
export async function buildUniversalOntologyMcpApplicationBundle() {
  const [publicPackage, noticesText] = await Promise.all([
    readJsonDocument(PUBLIC_PACKAGE_JSON_PATH),
    nodeFileSystem.readFile(PUBLIC_PACKAGE_NOTICES_PATH, "utf8"),
  ]);
  if (
    typeof publicPackage.version !== "string" ||
    publicPackage.version === ""
  ) {
    throw new Error("The public MCP package must declare its version.");
  }
  ...
  try {
    ...
    const [directBundledComponents, prebundledServerComponents] =
      await Promise.all([
        readBundledComponents(bundledPackages),
        readAndValidatePrebundledServerComponents(bundledPackages),
      ]);
    ...
    await assertNoticesCoverBundledComponents(
      noticesText,
      bundledComponents,
      bundledPackages,
    );
