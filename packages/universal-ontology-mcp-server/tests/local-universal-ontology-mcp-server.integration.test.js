...
import { UNIVERSAL_ONTOLOGY_MCP_REQUEST_BODY_MAXIMUM_BYTES } from "../src/universalOntologyMcpHttpHandlers.js";
...
function spawnShutdownFixture() {
  const child = spawn(
    ...
    [
      fileURLToPath(
        new URL(
          "./fixtures/localOntologyMcpShutdownFixture.js",
          import.meta.url,
        ),
      ),
    ],
...
describe("local Universal Ontology MCP server socket seam", () => {
  ...
  test("enforces representation, JSON, batch, and byte bounds on the real listener", async () => {
    ...
    try {
      ...
      expect(incompleteAccept.status).toBe(200);
  ...
  test.each([
    ...
    "bounds chunked UTF-8 POST input at %i bytes",
    async (byteLength, expectedStatus) => {
      const { localServer, address } = await startServer();
      const message = modernEnvelope(61, "tools/list");
      message.params._meta["example.test/padding"] = "é";
      const encodedMessage = JSON.stringify(message);
      const body =
        encodedMessage +
        " ".repeat(byteLength - Buffer.byteLength(encodedMessage));
      expect(Buffer.byteLength(body)).toBe(byteLength);
      expect(body.length).toBeLessThan(byteLength);
      try {
        const response = await rawHttpRequest({
          port: address.port,
          headers: {
            ...modernFetchOptions(61, "tools/list").headers,
  ...
  test("rejects unsupported methods before native conversion can consume their bodies", async () => {
    const ontologyQuery = createOntologyQueryStub();
    const { localServer, address } = await startServer({ ontologyQuery });
    let nativeBodyReads = 0;
    // Observe the real IncomingMessage iterator without changing its data or
    // return values. The supported SDK converter otherwise consumes PUT bodies.
    localServer.httpServer.prependListener("request", (request) => {
      const originalIterator = request[Symbol.asyncIterator];
      request[Symbol.asyncIterator] = function () {
        nativeBodyReads += 1;
    ...
    try {
      ...
      }).toEqual({ nativeBodyReads: 0, connection: "close", allow: "POST" });
      expect(ontologyQuery.searchOntologyEntities).not.toHaveBeenCalled();
    } finally {
      await localServer.shutdown();
    }
