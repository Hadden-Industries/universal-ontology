...
import { calculateSha256 } from "../src/ontologyQueryArtifactCanonicalBytes.js";
import { OntologyQueryArtifactCacheInitializationError } from "../src/ontologyQueryArtifactCacheInitializationErrors.js";
import {
  parseOntologyQueryChannelLastKnownGoodStateBytes,
  serializeCanonicalOntologyQueryChannelLastKnownGoodState,
} from "../src/ontologyQueryPersistentCacheSchemas.js";
import { createPersistentOntologyQueryArtifactCache } from "../src/persistentOntologyQueryArtifactCache.js";
...
const PERSISTENT_CACHE_WORKER_PATH = fileURLToPath(
  new URL("./fixtures/persistent-cache-worker.js", import.meta.url),
);
...
describe("persistent ontology query-artifact cache initialization", () => {
  ...
  test("initializes while another cache removes its completed capability probe", async () => {
    const { parent, cacheRoot } = await createTemporaryCacheRoot(
      "uo-cache-probe-race-",
    );
    const absoluteParent = resolve(parent);
    if (
      dirname(absoluteParent) !== resolve(tmpdir()) ||
      !basename(absoluteParent).startsWith("uo-cache-probe-race-")
    ) {
      throw new Error("Refusing cleanup outside the owned probe-race fixture.");
    }
    const temporaryDirectoryPath = join(
      getRepositoryRootPath(cacheRoot),
      "temporary",
    );
    const ownerProbePaths = ["linked", "source", "sentinel"].map((kind) =>
      join(temporaryDirectoryPath, `capability-probe-concurrent-owner-${kind}`),
    );
    const probeReady = Promise.withResolvers();
    const allowOwnerCleanup = Promise.withResolvers();
    const removedOwnerProbePaths = [];
    let enumeratedOwnerProbe = false;
    let ownerInitialization;
    ...
    try {
      ...
      ownerInitialization = createPersistentOntologyQueryArtifactCache({
        ontologyQueryArtifactCacheDirectoryPath: cacheRoot,
        ontologyQueryArtifactBaseUrlSha256: BASE_URL_SHA_256,
        randomIdentifier: () => "concurrent-owner",
        fileSystem: {
          ...nodeFileSystem,
          async unlink(path) {
            if (path === ownerProbePaths[0]) {
              probeReady.resolve();
              await allowOwnerCleanup.promise;
            }
            await nodeFileSystem.unlink(path);
            removedOwnerProbePaths.push(path);
          },
        },
      });
      await Promise.race([probeReady.promise, ownerInitialization]);

      const observerCache = await createPersistentOntologyQueryArtifactCache({
        ontologyQueryArtifactCacheDirectoryPath: cacheRoot,
        ontologyQueryArtifactBaseUrlSha256: BASE_URL_SHA_256,
        fileSystem: {
          ...nodeFileSystem,
          async readdir(path, options) {
            const entries = await nodeFileSystem.readdir(path, options);
            if (path === temporaryDirectoryPath) {
              // Return a real directory observation, then let the other cache
              // finish its normal cleanup before this cache inspects entries.
              expect(
                entries.some(
                  ({ name }) => name === basename(ownerProbePaths[0]),
                ),
              ).toBe(true);
              expect(
                (await nodeFileSystem.lstat(ownerProbePaths[0])).isFile(),
              ).toBe(true);
              enumeratedOwnerProbe = true;
              allowOwnerCleanup.resolve();
              await ownerInitialization;
              expect(removedOwnerProbePaths).toEqual(ownerProbePaths);
              await expect(
                nodeFileSystem.lstat(ownerProbePaths[0]),
      ...
      expect(enumeratedOwnerProbe).toBe(true);
      const artifact = await createArtifactFixture();
      await observerCache.installVerifiedArtifact(artifact);
      await expect(
        observerCache.readVerifiedArtifact(artifact),
      ).resolves.toEqual(artifact.bytes);
      expect(await nodeFileSystem.readdir(temporaryDirectoryPath)).toEqual([]);
    } finally {
      allowOwnerCleanup.resolve();
      if (ownerInitialization) {
        await ownerInitialization.catch(() => {});
      }
      await nodeFileSystem.rm(absoluteParent, { recursive: true, force: true });
    }
  });

  test("keeps metadata permission errors fatal during cache tree verification", async () => {
    const { parent, cacheRoot } = await createTemporaryCacheRoot(
      "uo-cache-metadata-denied-",
    );
    const absoluteParent = resolve(parent);
    if (
      dirname(absoluteParent) !== resolve(tmpdir()) ||
      !basename(absoluteParent).startsWith("uo-cache-metadata-denied-")
    ) {
      throw new Error(
        "Refusing cleanup outside the owned metadata-denial fixture.",
      );
    }
    ...
    const permissionError = Object.assign(
      new Error("private metadata access denied"),
      { code: "EACCES" },
    );
    ...
    try {
      ...
      await nodeFileSystem.writeFile(
        deniedPath,
        "existing private cache entry",
        { mode: 0o600 },
      );
      expect((await nodeFileSystem.lstat(deniedPath)).isFile()).toBe(true);
      const error = await expectInitializationFailure(
        createPersistentOntologyQueryArtifactCache({
          ...
          fileSystem: {
            ...nodeFileSystem,
            async lstat(path) {
              // Inject a filesystem permission outcome at the actual entry;
              // Windows ACL changes are neither needed nor simulated by modes.
              if (path === deniedPath) {
