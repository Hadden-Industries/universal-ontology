...
async function verifyManagedTree({
  ...
  for (const entry of entries) {
    ...
    const stats = await readStatsIfPresent(fileSystem, entryPath);

    // Another cache can remove its completed probe or temporary artifact after
    // enumeration. An absent entry is harmless; inspect every remaining entry.
    if (!stats) {
      continue;
    }
