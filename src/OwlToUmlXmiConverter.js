...
export class OwlToUmlXmiConverter {
    ...
    constructor(xmlString) {
        ...
        const parserError = this.sourceDoc.getElementsByTagNameNS(
    ...
    _walkNode(node) {
        ...
        if (
            ...
            (ns === rules.rdfs && localName === 'subClassOf' && resource && !resource.includes('http://www.w3.org/2001/XMLSchema#')) ||
            (ns === rules.rdfs && localName === 'domain' && resource) ||
            (ns === rules.rdfs && localName === 'range' && resource && !resource.includes('http://www.w3.org/2001/XMLSchema#')) ||
    ...
    convert() {
        ...
        const titleNodes = ontologyNode ? Array.from(ontologyNode.children).filter(c => 
            (c.namespaceURI === rules.NAMESPACES.dcterms && c.localName === 'title') ||
            (c.namespaceURI === rules.NAMESPACES.rdfs && c.localName === 'label')
        ) : [];
        ...
        for (const [classUri, nodes] of this.indexes.classes.entries()) {
            ...
            const superClasses = Array.from(defNode.children).filter(c => 
    ...
    _getClassName(defNode, classUri) {
        ...
        const nodes = Array.from(defNode.children).filter(c => 
