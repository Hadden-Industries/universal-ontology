...
class Policy:
    ...
    def requirement_ids(self) -> dict[URIRef, str]:
        ...
        return {
            node: str(value)
            for node, value in self.rules.subject_objects(UOP.requirementId)
        }
...
def load_policy(policy_directory: Path = POLICY_DIRECTORY) -> Policy:
    ...
    for graph_name in (CONTEXT_SHAPES_FILENAME, POLICY_METADATA_SHAPES_FILENAME):
        digest.update(
            ...
            + b"\0"
            + (policy_directory / graph_name).read_bytes()
            + b"\0"
        )
    policy = Policy(
        policy_directory,
...
def _prove_policy_contract(policy: Policy) -> None:
    conforms, _, text = pyshacl.validate(
        policy.rules,
        shacl_graph=policy.metadata_shapes,
        advanced=True,
        meta_shacl=True,
        inference="none",
    )
...
def _shacl_vocabulary() -> set[URIRef]:
    ...
    graph = Graph().parse(
        Path(pyshacl.__file__).parent / "assets" / "shacl.ttl", format="turtle"
    )
    return {
        subject
        for subject in graph.subjects()
        if isinstance(subject, URIRef) and str(subject).startswith(_SHACL_NAMESPACE)
    }
    ...
    for _, predicate, obj in rules:
        for term in (predicate, obj):
            if (
                ...
                and str(term).startswith(_SHACL_NAMESPACE)
                and term not in vocabulary
            ):
                unknown.add(str(term))
    if unknown:
        raise PolicyDefinitionError(
            f"Unknown SHACL vocabulary in policy (typo?): {sorted(unknown)}"
        )
