...
class ValidationOutcome:
    ...
    comparisons: tuple[
        tuple[str, str | None], ...
    ] = ()  # (module IRI, "locator digest" or None)
    ...
    def unevaluated_change_obligations(self) -> tuple[str, ...]:
        ...
        return tuple(
            module for module, compared in self.comparisons if compared is None
        )


def _prove_context(context: Graph, policy: Policy) -> None:
    conforms, _, text = pyshacl.validate(
        context, shacl_graph=policy.context_shapes, advanced=True, inference="none"
    )
    if not conforms:
...
def required_authorities(policy: Policy) -> tuple[str, ...]:
    return tuple(
        sorted(str(name) for name in policy.rules.objects(None, UOP.snapshotName))
    )
...
def validate_sources(
    ...
    if purpose == RunPurpose.CANDIDATE:
        missing = [
            str(s.module.iri) for s in sources if comparisons.get(s.module.iri) is None
        ]
        if missing:
            raise ContextError(
                f"Candidate qualification requires a comparison snapshot for every module; missing: {missing}"
            )
    if purpose == RunPurpose.CRITICAL_FIX and not scope_reference:
        raise ContextError(
            "A critical-fix run requires the explicitly approved scope reference."
        )
    ...
    try:
        ...
        raise PolicyDefinitionError(
            f"SHACL engine failure: {type(exc).__name__}: {exc}"
        ) from exc
    ...
    if targeted == 0:
        raise ContextError(
            "The policy selected zero focus nodes from a nonempty corpus; ownership or targets are broken."
        )
    return ValidationOutcome(
        purpose=purpose,
        policy_identity=policy.identity,
        module_identities=tuple(
            (str(s.module.iri), s.locator, s.digest) for s in sources
        ),
        ...
        comparisons=tuple(
            (
                str(s.module.iri),
                (
                    f"{comparisons[s.module.iri].locator} {comparisons[s.module.iri].digest}"
                    if comparisons.get(s.module.iri)
                    else None
                ),
            )
            for s in sources
...
def _restrict_to_fix_scope(results, context: Graph):
    ...
    in_scope = {
        str(s)
        for s, kind in context.subject_objects(UOC.changeKind)
        if kind in (UOC.Changed, UOC.Added)
    }
...
def _count_targeted(data: Graph, policy: Policy) -> int:
    ...
    return sum(
        ...
        if any((subject, RDF.type, kind) in data for kind in ENTITY_KINDS)
    )
