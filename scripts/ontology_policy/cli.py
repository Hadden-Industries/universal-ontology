...
class SelectedSource:
    ...
    comparison_revision: str | None = (
        None  # commit whose blob of the same path is the comparison; None: the active artifact
    )
...
def assemble_sources(
    ...
    if purpose != RunPurpose.LATEST_ACTIVE:
        for item in selected:
            ...
            if module is None:
                raise ContextError(
                    f"{item.path} does not belong to a reviewed owned module (policy/activation.ttl)."
                )
            if module.iri in replacements:
                raise ContextError(
                    f"Two selected inputs replace the same module {module.label}: {replacements[module.iri].path} and {item.path}."
                )
    ...
    for module in modules:
        ...
        if item is not None:
            sources.append(
                read_module_source(
                    module, item.path, revision=item.revision, repository=repository
                )
            )
            ...
            raise ContextError(
                f"{module.label} has no active artifact and no selected replacement."
            )
    return sources


def active_artifact_source(
    module: OwnedModule, repository: Path | None = None
) -> ModuleSource:
    """Read the activated artifact and refuse bytes that differ from the digest recorded at activation."""
    source = read_module_source(
        module, module.active_artifact_path, repository=repository
    )
...
def assemble_comparisons(
    ...
    for item in selected:
        ...
        if item.comparison_revision is not None:
            try:
                comparisons[module.iri] = read_module_source(
                    module,
                    item.path,
                    revision=item.comparison_revision,
    ...
    for module in modules:
        if comparisons[module.iri] is None and module.iri not in {
            module_for_path(modules, i.path).iri
            for i in selected
            if module_for_path(modules, i.path)
        }:
...
def draft_passes(
    ...
    for module_iri, item in extras:
        passes.append(
            [
                item if module_for_path(modules, i.path).iri == module_iri else i
                for i in passes[0]
            ]
        )
    return passes
...
def run_policy_validation(
    ...
    *,
    ...
    if purpose == RunPurpose.DRAFT and not selected:
        print(
            "Draft diagnostics: no ontology source selected; nothing validated.",
            file=stream,
        )
        return 0
    status = 0
    try:
        ...
        passes = (
            draft_passes(selected, modules, repository)
            if purpose == RunPurpose.DRAFT
            else [selected]
        )
        if len(passes) > 1:
            print(
                f"Draft diagnostics: {len(passes)} passes because a module is selected through more than one document.",
                file=stream,
            )
        for chosen in passes:
            ...
            outcome = validate_sources(
                ...
                purpose,
                policy,
                authorities_directory=authorities_directory,
            ...
            print(
                f"Report: {written.summary_path}"
                + (
                    f"; receipt: {written.receipt_path}" if written.receipt_path else ""
                ),
                file=stream,
            )
        ...
        print(
            f"POLICY_VALIDATION_ERROR ({type(error).__name__}): {error}",
            file=sys.stderr,
        )
        return STATUS_ERROR
    return status
...
def present(outcome: ValidationOutcome, *, github_actions: bool, stream) -> None:
    print(
        f"Run purpose: {outcome.purpose.value}; policy {outcome.policy_identity}",
        file=stream,
    )
    ...
    for module_iri, compared in outcome.comparisons:
        print(
            f"  comparison {module_iri}: {compared or 'unavailable (change obligations unevaluated)'}",
            file=stream,
        )
    if outcome.scope_reference:
        print(
            f"Critical-fix scope: {outcome.scope_reference}; results limited to changed subjects and affected referrers.",
            file=stream,
        )
    ...
    if outcome.results:
        ...
        if github_actions:
            for result in outcome.results:
                print(
                    github_annotation(
                        "error" if result.blocks else "warning",
