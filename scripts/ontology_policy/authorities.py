...
AUTHORITY_SPECIFICATIONS: dict[str, AuthoritySpecification] = {
    "iana-media-types": AuthoritySpecification(
        ...
        "application/xml",
        "IANA Media Types registry; a member is https://www.iana.org/assignments/media-types/<template>",
    ),
    "loc-iso639-1": AuthoritySpecification(
        "loc-iso639-1",
        "https://id.loc.gov/vocabulary/iso639-1.rdf",
        "loc-iso639-1.rdf",
        "application/rdf+xml",
        "Library of Congress ISO 639-1 language vocabulary; members are its skos:Concept subjects",
    ),
    "eu-file-type": AuthoritySpecification(
        ...
        "eu-file-type.rdf",
        "application/rdf+xml",
        "EU Vocabularies File Type authority table; members are subjects in the file-type scheme",
    ),
}
...
def _members_rdf(raw: bytes, *, select, description: str) -> tuple[set[str], dict]:
    ...
    try:
        ...
        raise AuthorityError(
            f"{description} payload is not parseable RDF/XML: {exc}"
        ) from exc
...
def _members_loc(raw: bytes):
    return _members_rdf(
        ...
        select=lambda graph: (
            ...
            if str(s).startswith(_LOC_MEMBER_PREFIX)
        ),
...
def _members_eu(raw: bytes):
    return _members_rdf(
        ...
        select=lambda graph: (
            s
            for s in graph.subjects(SKOS.inScheme, _EU_SCHEME)
...
_EXTRACTORS = {
    "iana-media-types": _members_iana,
    "loc-iso639-1": _members_loc,
    "eu-file-type": _members_eu,
}
...
def _members_turtle(specification: AuthoritySpecification, members: set[str]) -> bytes:
    ...
    lines.extend(
        f"<{member}> <{UOP.memberOf}> <{specification.iri}> ."
...
def derive_snapshot(name: str, raw: bytes, directory: Path, rights: dict) -> Snapshot:
    ...
    provenance = {
        ...
        "parserVersion": rdflib.__version__
        if name != "iana-media-types"
        else defusedxml.__version__,
        "transformation": {
            "iana-media-types": "template file names prefixed with the registry URL",
            "loc-iso639-1": "skos:Concept subjects under the iso639-1 namespace",
            "eu-file-type": "subjects with skos:inScheme file-type",
        }[name],
...
def reconcile_snapshot(name: str, raw: bytes, directory: Path) -> tuple[str, ...]:
    ...
    if provenance.get("derivedSha256") != hashlib.sha256(derived).hexdigest():
        problems.append(
            f"{name}: derived members file differs from the recorded derivedSha256"
        )
    members, _ = _EXTRACTORS[name](raw)
    if _members_turtle(AUTHORITY_SPECIFICATIONS[name], members) != derived:
        problems.append(
            f"{name}: regenerating from the raw payload does not reproduce the stored members"
        )
    if provenance.get("memberCount") != len(members):
        problems.append(
            f"{name}: recorded memberCount {provenance.get('memberCount')} differs from {len(members)}"
        )
    return tuple(problems)


def load_authority_graph(
    directory: Path = AUTHORITIES_DIRECTORY, required=tuple(AUTHORITY_SPECIFICATIONS)
) -> Graph:
    ...
    for name in required:
        ...
        if not members_path.is_file() or not provenance_path.is_file():
            raise AuthorityError(
                f"Authority snapshot {name} is missing from {directory}; validation cannot proceed."
            )
        provenance = json.loads(provenance_path.read_text(encoding="utf-8"))
        derived = members_path.read_bytes()
        if hashlib.sha256(derived).hexdigest() != provenance.get("derivedSha256"):
            raise AuthorityError(
                f"Authority snapshot {name} does not match its provenance digest."
            )
        part = Graph()
        try:
            ...
            raise AuthorityError(
                f"Authority snapshot {name} is not parseable: {exc}"
            ) from exc
        count = sum(
            1 for _ in part.subjects(UOP.memberOf, AUTHORITY_SPECIFICATIONS[name].iri)
        )
        if count != provenance.get("memberCount") or count == 0:
            raise AuthorityError(
                f"Authority snapshot {name} has {count} members but provenance records {provenance.get('memberCount')}."
            )
        graph += part
    return graph
...
def authority_identities(
    ...
    for name in required:
        provenance = json.loads(
            (directory / f"{name}.provenance.json").read_text(encoding="utf-8")
        )
