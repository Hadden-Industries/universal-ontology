...
import pyshacl
import rdflib
from rdflib import SH, BNode, Graph, Literal
...
def extract_results(results_graph: Graph, policy: Policy) -> list[PolicyResult]:
    ...
    top_level = [
        result
        for report in results_graph.subjects(SH.conforms, None)
    ...
    for result in top_level:
        ...
        messages = sorted(
            str(value) for value in results_graph.objects(result, SH.resultMessage)
        )
        results.append(
            PolicyResult(
...
def format_results(results: list[PolicyResult]) -> str:
    ...
    for result in results:
        lines.append(
            f"{result.severity:<9} {result.requirement_id:<28} {result.focus_node}"
        )
        if result.path:
            lines.append(
                f"          path {result.path}"
...
def policy_anchor(
    requirement_id: str, name: str, strength: str, human: bool = False
) -> str:
    """GitHub heading anchor of the rule in the generated policy document."""
    heading = (
        f"{requirement_id} — {name} ({strength}{', human review' if human else ''})"
    )
...
def github_annotation(
    level: str, requirement_id: str, focus_node: str, message: str
) -> str:
...
def report_document(outcome, policy) -> dict:
    ...
    def describe(requirement_id: str) -> dict:
        ...
        return {
            "policyAnchor": policy_anchor(requirement_id, name, strength, human),
            "repairGuidance": str(policy.rules.value(node, SH.description) or ""),
        }
    ...
    return {
        ...
        "modules": [
            {"module": module, "locator": locator, "digest": digest}
            for module, locator, digest in outcome.module_identities
        ],
        "authorities": [
        ...
        "comparisons": [
            {"module": module, "comparedWith": compared}
            for module, compared in outcome.comparisons
        ],
        ...
        "results": [
            {
                ...
                "path": r.path,
                "value": r.value,
                "constraintComponent": r.constraint_component,
                "message": r.message,
                "sourceShape": r.source_shape,
                **describe(r.requirement_id),
            }
...
def write_report(
    outcome, directory: Path = REPORT_DIRECTORY, policy=None
) -> WrittenReport:
    ...
    graph_path.write_bytes(
        outcome.results_graph.serialize(format="turtle").encode("utf-8")
    )
    receipt_path = None
