    ...
    *,
    ...
    if not receipt_path.is_file():
        raise PublicationRefusal(
            f"No qualification receipt at {receipt_path}; run the latest-active or candidate qualification first."
        )
    ...
    if receipt.get("purpose") not in ("latest-active", "candidate"):
        raise PublicationRefusal(
            f"The receipt purpose {receipt.get('purpose')!r} is diagnostic; a qualification purpose is required."
        )
    if receipt.get("policyIdentity") != policy_identity:
        raise PublicationRefusal(
            "The policy changed since qualification; re-qualify before publishing."
        )
    recorded_authorities = {
        (a["name"], a["digest"]) for a in receipt.get("authorities", [])
    }
    if recorded_authorities != set(authority_identities):
        raise PublicationRefusal(
            "The authority snapshots changed since qualification; re-qualify before publishing."
        )
    if receipt.get("lockIdentity") != lock_identity:
        raise PublicationRefusal(
            "The dependency lock changed since qualification; re-qualify before publishing."
        )
    ...
    for module_iri, artifact in sorted(active_artifacts.items()):
        ...
        if entry is None:
            raise PublicationRefusal(
                f"Module {module_iri} is not covered by the receipt; the module set is only partly prepared."
            )
        if entry["locator"] != artifact:
            raise PublicationRefusal(
                f"Module {module_iri} was qualified as {entry['locator']} but the active artifact is {artifact}."
            )
        ...
        if current != entry["digest"]:
            raise PublicationRefusal(
                f"{artifact} differs from the qualified bytes ({entry['digest']} recorded, {current} now)."
            )
        derived = (
            dist_directory / artifact.split("/", 1)[1]
            if artifact.startswith("src/")
            else dist_directory / artifact
        )
        if not derived.is_file() or derived.read_bytes() != source.read_bytes():
            raise PublicationRefusal(
                f"The derived output {derived} does not match the qualified source bytes of {artifact}; rebuild before publishing."
            )
...
def check_repository_publication(
    ...
    active = {
        str(m.iri): m.active_artifact_path
    ...
    return check_publication_gate(
        ...
        authority_identities=authority_identities(authorities_directory, required)
        if required
        else (),
