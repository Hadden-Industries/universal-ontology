...
def extract_lexical_boundaries(xml_text: str, root_tag: str) -> tuple[str, str, str]:
    ...
    if not pre_match or not post_match:
        raise ValueError(
            "Failed to match structural boundaries of the root element. File may use self-closing root tags."
        )
...
def atomically_write_string(payload: str, target_file_path: str) -> None:
    ...
    with tempfile.NamedTemporaryFile(
        ...
        try:
            ...
            raise RuntimeError(
                "Serialization of patched string to staging file failed."
            ) from write_error
    ...
    try:
        ...
        os.remove(staging_file_path)
        raise RuntimeError(
            "Atomic replacement of target XML file failed."
        ) from replacement_error


def atomically_write_xml(
    transformed_dom: etree._XSLTResultTree, target_file_path: str
) -> None:
    """
    ...
    directory_path = os.path.dirname(target_file_path) or "."
    with tempfile.NamedTemporaryFile(
        delete=False, dir=directory_path, suffix=".xml"
    ) as staging_file:
        staging_file_path = staging_file.name
        try:
            ...
            os.remove(staging_file_path)
            raise RuntimeError(
                "DOM Serialization to staging file failed."
            ) from write_error
...
def apply_xslt_in_place(xml_file_path: str, xslt_file_path: str) -> None:
    ...
    try:
        ...
        orig_preamble, _, orig_postamble = extract_lexical_boundaries(
            original_text, root_tag_name
        )

        transformed_text = bytes(transformed_xml_dom).decode("utf-8")
        _, trans_interior, _ = extract_lexical_boundaries(
            transformed_text, root_tag_name
        )
        ...
        logging.warning(
            "Lexical mapping failed. Degraded to standard DOM serialization. Reason: %s",
            str(lexical_error),
        )
        try:
            ...
            logging.error(
                "Failed to commit fallback DOM changes to the filesystem safely."
            )
            sys.exit(1)
    except RuntimeError:
        logging.error(
            "Failed to commit patched string changes to the filesystem safely."
        )
        sys.exit(1)

    logging.info(
        "Successfully transformed '%s' in-place using '%s'.",
        xml_file_path,
        xslt_file_path,
    )
