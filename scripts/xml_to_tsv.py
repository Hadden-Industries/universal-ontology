...
class OntologyConfiguration:
    ...
    FIELDS: List[Dict[str, str]] = [
        ...
        {"header_name": "URI"},
        {
            "header_name": "Preferred Label",
            "tag": "{http://www.w3.org/2004/02/skos/core#}prefLabel",
        },
        {
            "header_name": "Definition",
            "tag": "{http://www.w3.org/2004/02/skos/core#}definition",
        },
        ...
        {
            "header_name": "SubClassOf",
            "tag": "{http://www.w3.org/2000/01/rdf-schema#}subClassOf",
        },
        {
            "header_name": "Class of Named Individual",
...
class OntologyExtractor:
    ...
    def extract_preferred_language(
        element: Any, target_tag: str, preferred: str = "en-GB", fallback: str = "en"
    ) -> str:
...
class OntologyProcessor:
    ...
    def _build_axiom_index(self, root: Any) -> Dict[str, str]:
        ...
        for axiom in root.iterfind(
            OntologyConfiguration.OWL_AXIOM, OntologyConfiguration.NAMESPACES
        ):
            annotated_source = axiom.find(
                OntologyConfiguration.OWL_ANNOTATED_SOURCE,
                OntologyConfiguration.NAMESPACES,
            )
            annotated_property = axiom.find(
                OntologyConfiguration.OWL_ANNOTATED_PROPERTY,
                OntologyConfiguration.NAMESPACES,
            )
            ...
            if annotated_source is not None and annotated_property is not None:
                ...
                property_uri = annotated_property.get(
                    OntologyConfiguration.RDF_RESOURCE
                )

                if (
                    property_uri == OntologyConfiguration.SKOS_DEFINITION_URI
                    and source_uri
                ):
                    source_resources = []
                    for dcterms_src in axiom.iterfind(
                        OntologyConfiguration.DCTERMS_SOURCE,
                        OntologyConfiguration.NAMESPACES,
                    ):
                    ...
                    if source_resources:
                        concatenated_sources = (
                            OntologyConfiguration.TSV_TERMINATOR.join(source_resources)
                        )
                        if source_uri in axiom_map:
                            axiom_map[source_uri] += (
                                OntologyConfiguration.TSV_TERMINATOR
                                + concatenated_sources
                            )
                        else:
    ...
    def _compile_record_vector(
        self,
        element: Any,
        object_type: str,
        record_uri: str,
        axiom_index: Dict[str, str],
    ) -> List[str]:
        ...
        for field in OntologyConfiguration.FIELDS:
            ...
            try:
                if header == "Object Type":
                    ...
                    for identifier in element.iterfind(
                        tag, OntologyConfiguration.NAMESPACES
                    ):
                        res = identifier.get(OntologyConfiguration.RDF_RESOURCE)
                        if res and res.startswith(OntologyConfiguration.UUID_PREFIX):
                            extracted_value = res[
                                len(OntologyConfiguration.UUID_PREFIX) :
                            ]
                            break
                elif header in ("Preferred Label", "Definition") and tag:
                    extracted_value = OntologyExtractor.extract_preferred_language(
                        element, tag
                    )
                elif header == "Sources" and tag:
                    if record_uri.startswith(
                        "https://haddenindustries.com/ontology/iso"
                    ):
                        identifier_tag = "{http://purl.org/dc/terms/}identifier"
                        for identifier in element.iterfind(
                            identifier_tag, OntologyConfiguration.NAMESPACES
                        ):
                            res = identifier.get(OntologyConfiguration.RDF_RESOURCE)
                            text_val = (
                                identifier.text.strip() if identifier.text else ""
                            )
                    ...
                    if not extracted_value:
                        extracted_value = OntologyExtractor.extract_resource_attribute(
                            element, tag
                        )
                        if not extracted_value and record_uri in axiom_index:
                            extracted_value = axiom_index[record_uri]
                elif (
                    header in ("Creator", "SubClassOf", "Class of Named Individual")
                    and tag
                ):
                    extracted_value = OntologyExtractor.extract_resource_attribute(
                        element, tag
                ...
                logger.warning(
                    f"Field extraction failure: '{header}' on URI '{record_uri}' - {str(ex)}"
                )
                extracted_value = ""
    ...
    def _process_document(self, file_path: Path, tsv_writer: Any) -> None:
        ...
        try:
            ...
            for element in root:
                ...
                object_type = (
                    "Class"
                    if local_tag_name == "Class"
                    else "Named Individual"
                ...
                if local_tag_name == "NamedIndividual":
                    skip_entity = False
                    for type_node in element.iterfind(
                        OntologyConfiguration.RDF_TYPE, OntologyConfiguration.NAMESPACES
                    ):
                        if (
                            type_node.get(OntologyConfiguration.RDF_RESOURCE)
                            in OntologyConfiguration.EXCLUDED_DCAT_TYPES
                        ):
                            skip_entity = True
                            break
                    if skip_entity:
                ...
                row_vector = self._compile_record_vector(
                    element, object_type, record_uri, axiom_index
                )
            ...
            with open(self.output_path, "w", newline="", encoding="utf-8") as tsvfile:
                ...
                for input_path in self.input_paths:
                    if not input_path.exists():
                        logger.warning(
                            f"File unresolvable. Skipping target: {input_path}"
                        )
                        self.failed_files += 1
                        continue
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Secure XML Ontology to TSV Transformer."
    )
    parser.add_argument(
        "input_file_paths", type=Path, nargs="+", help="One or more target XML files."
    )
    parser.add_argument(
        "output_file_path",
        type=Path,
        help="Absolute or relative path to the destination TSV.",
    )
