...
**Design-review prototype:** based on the supplied September snapshot; not deployed. See the design pack for provenance and verification.

**[Contributor guide](#contributor-guide)** — understand what to contribute and what your edit needs.  
**[Technical reference](#technical-reference)** — inspect every exact rule, exception, check and source.

The same policy applies to both reading paths. The guide is an explanation, not a weaker conformance profile.

<a name="contributor-guide"></a>
## Contributor guide

For subject-matter contributors and anyone who wants to improve the meaning of the ontology. Start with your change; use the reference for exact representation rules.

### Start with the meaning

Search the latest active ontologies before adding a Class, NamedIndividual or property. Look for the same meaning, including synonyms and alternative spellings. Reuse an existing concept when it fits. When a broader concept exists, place the new concept beneath it. Describe the search and your judgement in the pull request.

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="assets/contribution-journey-dark.png">
  <img src="assets/contribution-journey-light.png" alt="Search for existing concepts, reuse or place the concept, describe the proposed edit, then combine automated structural checks with human review." width="980">
</picture>

**In words:** search → reuse or place the concept → describe the edit → check its structure and review its meaning. Passing automated checks does not establish that a concept is new or that its definition is adequate.

[Exact concept-reuse requirement](#ep-human-concept-reuse).

### Choose the kind of edit

| Your change | Where to start |
| --- | --- |
| Add or clarify a Class, NamedIndividual or property | [Ordinary entity checklist](#ordinary-entity-checklist) |
| Change an existing ordinary entity | [Modification and attribution](#modification-and-attribution) |
| Describe a dataset or a way to access it | [Dataset and distribution guide](#dataset-and-distribution-guide) |
| Change a module’s version or an axiom annotation | [Ontology header](#ontology-header) or [Axiom annotations](#axiom-annotations) in the reference |

<a name="ordinary-entity-checklist"></a>
### Ordinary entity checklist

This checklist concerns the owned Classes, NamedIndividuals, ObjectProperties and DatatypeProperties covered by the ordinary entity rules. It does **not** replace the separate dataset and distribution profiles.

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="assets/entity-anatomy-dark.png">
  <img src="assets/entity-anatomy-light.png" alt="Ordinary entity entries combine meaning, language-tagged names, identity, attribution, and change or source information. Definitions are required for Classes and NamedIndividuals; properties may omit them." width="980">
</picture>

**Meaning and names.** Give a Class or NamedIndividual a definition; a property may omit one. Tag definitions and names with their language. Include an English preferred label and copy each preferred label exactly into an ordinary label, preserving both its text and language tag. Preferred labels and definitions have at most one value per language tag. At least one English preferred label must correspond to the entry’s local name.

**Naming examples.** `RiskEvent` is a Class local name. `Red` or `Colour_Red` can name an individual explicitly typed as `Colour`. `hasPart` and `Thing_hasPart` are non-ISO property examples. ISO-naming modules instead recommend lower snake_case, such as `registration_status`; that recommendation is reported as a warning, not an automatic block. Follow the exact prefix and correspondence rules when using the less common forms.

**Identity and attribution.** Include at least one version-4 UUID URN identifier, one creator’s canonical ORCID IRI, and one creation timestamp in UTC ending with the literal `Z`, such as `2026-01-02T03:04:05Z`. IRI identifiers must not be shared by distinct owned entities in the validated module set. Other identifier kinds may accompany the UUID; the uniqueness rule does not apply to literal enumeration codes.

**Sources and extra description.** Cite an externally derived definition using `dcterms:source` as recommended by the policy. References and see-also annotations are optional and repeatable. Tag descriptive text, including acronyms, with its language. An ordinary entity’s optional description has at most one value per language tag.

[Definitions](#ep-definition) · [Preferred labels](#ep-preflabel-language) · [Label matching](#ep-preflabel-label) · [Name correspondence](#ep-preflabel-iri) · [UUID identifiers](#ep-entity-uuid).

<a name="modification-and-attribution"></a>
### Modification and attribution

When an existing entity changes, it needs valid modification metadata under the comparison-based rule. Supply at most one modification value: an allowed date, or a date-time in UTC ending with `Z`. A new entity does not need a modification value. An existing valid value need not change in ordinary editing; the policy does not impose a fresh timestamp on every edit.

When someone other than the creator changes the entity, that person adds their ORCID as a contributor. Contributor values may repeat, and every supplied value must have the required format. The validator does not infer contributors from Git identity.

[Exact modification rule, including an unavailable comparison snapshot](#ep-modified) · [Contributor rule](#ep-contributor).

<a name="dataset-and-distribution-guide"></a>
### Dataset and distribution guide

A **dataset record** describes a collection of data. A **distribution record** describes an available distribution of that dataset. An access URL can lead to a page explaining how to obtain it; a direct file download is not mandatory.

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="assets/dataset-access-guide-dark.png">
  <img src="assets/dataset-access-guide-light.png" alt="A dataset record may link to zero or more distribution records. Each distribution has at least one access URL. A direct download URL is optional. Dataset and distribution metadata follow separate profiles." width="980">
</picture>

A dataset needs explicit dataset and NamedIndividual types, its dedicated UUID-based IRI, at least one theme, and non-blank language-tagged title, description and label values. Descriptions are limited to one per language tag. A dataset may link to several distributions or none.

A distribution needs explicit distribution and NamedIndividual types, its dedicated UUID-based IRI, one or more IRI access URLs, and one or more non-blank language-tagged labels. A linked distribution must be explicitly declared in the local validated module set; an online page does not supply that declaration.

Optional fields still have rules. A supplied media type or distribution language must belong to the relevant pinned vocabulary snapshot. Other fields have their own cardinality, IRI or recommendation requirements. Use the distribution reference when supplying them. A dataset landing page should belong to the original provider; that is a human judgement.

[Dataset reference](#datasets) · [Distribution reference](#distributions).

### Before submitting the edit

Check the automated results and address failures. Consider warnings rather than treating them as either automatic rejection or permission to ignore a recommendation. In the pull request, describe the concept-reuse search and the human judgements relevant to your change. Review definition adequacy, contributor attribution, external derivation and original-provider landing pages when applicable.

> [!IMPORTANT]
> A graph can pass every executable rule and still duplicate an existing concept. Automated checking and human review are complementary, not interchangeable.

<details>
<summary>Vocabulary used on this page</summary>

**Owned** means the subject belongs to an authored module in the validation context, rather than merely appearing as an imported or foreign term. **Local name** is the part of an entity IRI after its final `/` or `#`. **IRI** is the resource identifier used by the graph; it is not interchangeable with a literal string containing a similar address. **Language tag** identifies the language of text, for example `@en` or `@en-GB`. **SHACL** is the constraint language used for structural validation. **Comparison snapshot** is the earlier state used to establish whether an existing entity changed. **Pinned vocabulary** is the specific local vocabulary snapshot used by the validation run.

</details>

[Continue to the complete technical reference](#technical-reference).
...
<summary>Source and original clause heading — EP-HUMAN-CONCEPT-REUSE</summary>
...
[Back to technical navigation](#technical-reference) · [Contributor guide](#contributor-guide)
...
<summary>Executable constraints — EP-CLASS-NAME</summary>
...
<summary>Source and original clause heading — EP-CLASS-NAME</summary>
...
<summary>Executable constraints — EP-INDIVIDUAL-NAME</summary>
...
<summary>Source and original clause heading — EP-INDIVIDUAL-NAME</summary>
...
<summary>Executable constraints — EP-PROPERTY-NAME</summary>
...
<summary>Source and original clause heading — EP-PROPERTY-NAME</summary>
...
<summary>Executable constraints — EP-PROPERTY-NAME-ISO</summary>
...
<summary>Source and original clause heading — EP-PROPERTY-NAME-ISO</summary>
...
<summary>Executable constraints — EP-LABEL</summary>
...
<summary>Source and original clause heading — EP-LABEL</summary>
...
<summary>Executable constraints — EP-PREFLABEL-LANGUAGE</summary>
...
<summary>Source and original clause heading — EP-PREFLABEL-LANGUAGE</summary>
...
<summary>Executable constraints — EP-PREFLABEL-IRI</summary>
...
<summary>Source and original clause heading — EP-PREFLABEL-IRI</summary>
...
<summary>Executable constraints — EP-PREFLABEL-LABEL</summary>
...
<summary>Source and original clause heading — EP-PREFLABEL-LABEL</summary>
...
<summary>Executable constraints — EP-DEFINITION</summary>
...
<summary>Source and original clause heading — EP-DEFINITION</summary>
...
<summary>Executable constraints — EP-DESCRIPTIVE-LANGUAGE</summary>
...
<summary>Source and original clause heading — EP-DESCRIPTIVE-LANGUAGE</summary>
...
<summary>Executable constraints — EP-DESCRIPTION-LANGUAGE</summary>
...
<summary>Source and original clause heading — EP-DESCRIPTION-LANGUAGE</summary>
...
<summary>Source and original clause heading — EP-ENTITY-UUID</summary>
...
<summary>Executable constraints — EP-IDENTIFIER-UNIQUE</summary>
...
<summary>Source and original clause heading — EP-IDENTIFIER-UNIQUE</summary>
...
<summary>Executable constraints — EP-ENTITY-CREATOR</summary>
...
<summary>Source and original clause heading — EP-ENTITY-CREATOR</summary>
...
<summary>Executable constraints — EP-ENTITY-CREATED</summary>
...
<summary>Source and original clause heading — EP-ENTITY-CREATED</summary>
...
<summary>Executable constraints — EP-MODIFIED</summary>
...
<summary>Source and original clause heading — EP-MODIFIED</summary>
...
<summary>Executable constraints — EP-CONTRIBUTOR</summary>
...
<summary>Source and original clause heading — EP-CONTRIBUTOR</summary>
...
<summary>Executable constraints — EP-OPTIONAL-ANNOTATIONS</summary>
...
<summary>Source and original clause heading — EP-OPTIONAL-ANNOTATIONS</summary>
...
<summary>Executable constraints — EP-DATASET-TYPE-IRI</summary>
...
<summary>Source and original clause heading — EP-DATASET-TYPE-IRI</summary>
...
<summary>Executable constraints — EP-DATASET-REQUIRED</summary>
...
<summary>Source and original clause heading — EP-DATASET-REQUIRED</summary>
...
<summary>Executable constraints — EP-DATASET-DISTRIBUTION</summary>
...
<summary>Source and original clause heading — EP-DATASET-DISTRIBUTION</summary>
...
<summary>Source and original clause heading — EP-DATASET-LANDING</summary>
...
<summary>Executable constraints — EP-DATASET-ACCESS-RIGHTS</summary>
...
<summary>Source and original clause heading — EP-DATASET-ACCESS-RIGHTS</summary>
...
<summary>Executable constraints — EP-DATASET-ACCESS-RIGHTS-IRI</summary>
...
<summary>Source and original clause heading — EP-DATASET-ACCESS-RIGHTS-IRI</summary>
...
<summary>Executable constraints — EP-DISTRIBUTION-REQUIRED</summary>
...
<summary>Source and original clause heading — EP-DISTRIBUTION-REQUIRED</summary>
...
<summary>Executable constraints — EP-DISTRIBUTION-DOWNLOAD</summary>
...
<summary>Source and original clause heading — EP-DISTRIBUTION-DOWNLOAD</summary>
...
<summary>Executable constraints — EP-DISTRIBUTION-MEDIA</summary>
...
<summary>Source and original clause heading — EP-DISTRIBUTION-MEDIA</summary>
...
<summary>Executable constraints — EP-DISTRIBUTION-FORMAT</summary>
...
<summary>Source and original clause heading — EP-DISTRIBUTION-FORMAT</summary>
...
<summary>Executable constraints — EP-DISTRIBUTION-FORMAT-EU</summary>
...
<summary>Source and original clause heading — EP-DISTRIBUTION-FORMAT-EU</summary>
...
<summary>Executable constraints — EP-DISTRIBUTION-LANGUAGE</summary>
...
<summary>Source and original clause heading — EP-DISTRIBUTION-LANGUAGE</summary>
...
<summary>Executable constraints — EP-DISTRIBUTION-LICENCE</summary>
...
<summary>Source and original clause heading — EP-DISTRIBUTION-LICENCE</summary>
...
<summary>Executable constraints — EP-DISTRIBUTION-RIGHTS</summary>
...
<summary>Source and original clause heading — EP-DISTRIBUTION-RIGHTS</summary>
...
<summary>Executable constraints — EP-ONT-VERSION</summary>
...
<summary>Source and original clause heading — EP-ONT-VERSION</summary>
...
<summary>Executable constraints — EP-AXIOM-POSITION</summary>
...
<summary>Source and original clause heading — EP-AXIOM-POSITION</summary>
