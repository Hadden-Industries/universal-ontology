...
- `universal_ontology` is this checkout's canonical single-file MCP application
  ...
  source—runs the existing authoritative `npm run generate:ontology-indexes` generator. Setup
...
It runs `npm run generate:ontology-indexes`, verifies a real query against `dist/query/v1`, and
...
`npm run serve:mcp-development`, keep a terminal open, or prefill an HTTP cache.
...
To read artifacts generated by `npm run generate:ontology-indexes` directly, select the
